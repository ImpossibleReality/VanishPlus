package codes.wickedtree.vanishplus.commands;
...
public final class VanishCommand {
    private static final VanishPlus vanish = VanishPlus.INSTANCE;
...
//        dispatcher.register(literal("v").redirect(commandNode));
    ...
    public static void sendFakePlayerListEntry(ServerPlayerEntity player) {
        if (VanishPlus.INSTANCE.getVanishedPlayers().stream().noneMatch(vanishedPlayer -> vanishedPlayer.getUuid().equals(player.getUuid()))) return;
